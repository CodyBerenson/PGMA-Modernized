    ...
    def update(self, metadata, media, lang):
        ...
        if nfo_file:
            ...
            if nfo_text_lower.count('<movie') > 0 and nfo_text_lower.count('</movie>') > 0:
                ...
                # Posters
                try:
                    posters = nfo_xml.xpath('poster')
                    for poster in posters:
                        poster = poster.text.strip()
                        log.info ('Found a poster: %s', poster)
                ...
                # Backgrounds
                try:
                    backgrounds = nfo_xml.xpath('background')
                    for background in backgrounds:
                        background = background.text.strip()
                        log.info ('Found a background: %s', background)
                        if background not in metadata.art:
                            metadata.art[background] = Proxy.Media(HTTP.Request(background).content, sort_order=1)
                except:
                    log.info('No background found...')
                    pass
    ...
    for count, url in enumerate(urlList, start=1):
        ...
        try:
            ...
            if 'gender=' in url:
                ...
                if FilmYear is None and len(actorList) == 1:
                    try:
