# QueerClick - (IAFD)
import datetime, linecache, platform, os, re, string, sys, urllib, urllib2, subprocess, traceback

# Version / Log Title 
VERSION_NO = '2020.02.14.6'
...
class QueerClick(Agent.Movies):
    ...
    def update(self, metadata, media, lang, force=True):
        ...
        try:
            ...
            # Poster - set first image as poster
            try:
                thumborImage = None
                scriptImage = None
            ...
            try:
                ...
                self.log('UPDATE:: Movie Background Art Found: size; (%sx%s) address; "%s"', width, height, image)
                # cropping needs to be done
                if height > desiredHeight:
                    thumborImage = THUMBOR.format(width, desiredHeight, image)
                    # if thumbor service is down - use vbscript (only windows)
            ...
            self.log('UPDATE:: Error getting Movie Poster and Background Art: %s', e)
            pass